...
namespace FlappyReDovahLauncher
{
    ...
    internal static class Loc
    {
        ...
        private static readonly Dictionary<string, string> EnMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "repair", "Repair" },
            { "open_game_dir", "Game folder" },
            { "open_user_mods", "User mods" },
            { "clear_shaders", "Clear shaders" },
            { "open_log", "Launcher log" },
            { "shaders_cleared_msg", "ENB and shader cache have been successfully cleared." },
            { "cancel", "Cancel" },
            { "get_vr", "Install VR" },
            { "remove_vr", "Remove VR" },
            ...
            { "desc_flappy", "Skyrim AE — closed test" },
            { "access_title", "Closed test" },
            { "access_hint", "{0} is in closed test.\nPaste the phrase from the Discord chat." },
            { "access_ok", "Continue" },
            { "access_bad", "That phrase does not match." },
            { "access_rate", "Too many tries. Wait {0} min." },
            { "access_needed", "Need the closed-test phrase to install." },
            { "access_offline", "Cannot reach the access server." },
            ...
            { "need_torrent_short", "Install from the torrent folder" },
            { "need_torrent", "Full download from CDN is disabled for now.\n\n1. Download the torrent. Keep the folder name Flappy\n2. Run Flappy Launcher.exe inside that folder\n3. The launcher unpacks the game from the torrent\n\nAfter that, Update and Repair work as usual (CDN)." },
            { "vr_unsupported", "This title has no VR pack." },
            { "tray_open", "Open" },
            { "tray_exit", "Exit" },
            { "tray_rebuild", "Rebuild Outputs" },
            ...
            { "sevenzip_missing", "7-Zip is not available.\n\nThe launcher downloads its own 7-Zip from https://www.7-zip.org/ (it does not use a system install).\nCheck your network and retry." },
            { "admin_title", "Administrator required" },
            { "admin_required", "Run this launcher as Administrator.\n\nMO2 tools need elevation:\n• Generate Output folders\n• RaceMenu hotkey R\n• writing the game path in the registry\n\nWithout Administrator rights the launcher will close." },
        ...
        private static readonly Dictionary<string, string> RuMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "modding", "Mod Organizer 2" },
            { "repair", "Починка" },
            { "open_game_dir", "Папка игры" },
            { "open_user_mods", "Мои моды" },
            { "clear_shaders", "Сброс шейдеров" },
            { "open_log", "Лог лаунчера" },
            { "shaders_cleared_msg", "Кэш шейдеров ENB и Community Shaders успешно очищен." },
            { "cancel", "Отмена" },
            ...
            { "desc_flappy", "Skyrim AE — закрытый тест" },
            { "access_title", "Закрытый тест" },
            { "access_hint", "{0} в закрытом тесте.\nВставьте фразу из Discord-чата." },
            { "access_ok", "Дальше" },
            { "access_bad", "Фраза не совпала." },
            { "access_rate", "Слишком много попыток. Подождите {0} мин." },
            { "access_needed", "Для установки нужна фраза закрытого теста." },
            { "access_offline", "Сервер доступа не отвечает." },
            { "cancelling", "Отмена…\nПодождите" },
            ...
            { "need_torrent_short", "Установка из папки торрента" },
            { "need_torrent", "Полная загрузка с CDN сейчас отключена.\n\n1. Скачайте торрент. Папку оставьте с именем Flappy\n2. Запустите Flappy Launcher.exe внутри неё\n3. Лаунчер распакует игру из торрента\n\nДальше Обновление и Починка работают как раньше (CDN)." },
            ...
            { "sevenzip_missing", "7-Zip недоступен.\n\nЛаунчер качает свою копию с https://www.7-zip.org/ (системный 7-Zip не используется).\nПроверьте сеть и повторите." },
            { "admin_title", "Нужны права администратора" },
            { "admin_required", "Запускайте лаунчер только от имени администратора.\n\nИнструменты MO2 требуют эти права:\n• Generate Output-папки\n• клавиша R в RaceMenu\n• запись пути к игре в реестре\n\nБез прав администратора лаунчер закроется." },
